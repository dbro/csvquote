...
outerloop:
		...
		// replace mode
		datatomap := make([]byte, 4096)
		datatowrite := make([]byte, 4096)
		c1 := make(chan int)
		c2 := make(chan int)
		go func() {
			for {
				counttomap := <- c1 // ready to map
				for i := 0; i < counttomap; i++ {
					datatowrite[i], stateQuoteInEffect, stateMaybeEscapedQuoteChar =
						mapFunction(datatomap[i], stateQuoteInEffect, stateMaybeEscapedQuoteChar)
				}
				c2 <- counttomap
			}
		}()
		go func() {
			for {
				counttowrite := <- c2 // ready to write
				os.Stdout.Write(datatowrite[:counttowrite])
			}
		}()
		for {
			if count, err := input.Read(data); err != nil {
